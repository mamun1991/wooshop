            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div class="flex flex-col gap-8 order-2 lg:order-1">
                <div class="flex flex-col gap-4">
                  <span class="text-primary font-bold tracking-widest uppercase text-sm">Season 2024 Collection index page</span>
              ...
              <div class="order-1 lg:order-2" x-data="carousel()" x-init="init()">
                <div class="relative">
                  ...
                  <div class="relative w-full aspect-[4/5] bg-slate-200 dark:bg-slate-800 rounded-2xl overflow-hidden shadow-2xl">
                    ...
                    <template x-for="(slide, index) in slides" :key="index">
                      <div 
                        class="absolute inset-0 w-full h-full bg-center bg-cover transition-transform duration-800 ease-out"
                        :style="`background-image: url('${slide.image}'); transform: translateX(${index === currentSlide ? '0%' : index > currentSlide ? '100%' : '-100%'})`"
                        :data-alt="slide.alt"
                      ></div>
                    </template>
                    ...
                    <button 
                      @click="prev()" 
                      class="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white/20 hover:bg-white/40 text-white rounded-full p-3 transition-all duration-300 backdrop-blur-sm"
                    ...
                    <button 
                      @click="next()" 
                      class="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white/20 hover:bg-white/40 text-white rounded-full p-3 transition-all duration-300 backdrop-blur-sm"
                    ...
                    <div class="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex gap-2">
                      <template x-for="(slide, index) in slides" :key="index">
                        <button 
                          ...
                          :class="currentSlide === index ? 'bg-white w-8' : 'bg-white/50 w-3 hover:bg-white/70'"
